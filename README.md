...
> アイキャッチ画像は `featured_media` に設定されるほか、**タイトルと本文の間（本文先頭）にも画像ブロックとして自動挿入**されます（テーマがアイキャッチを本文上に表示しない場合の対策）。本文先頭に入れたくない場合は `--no-inline-eyecatch` を指定してください。

> 本文は **Gutenberg ブロック形式**（段落・見出し・リスト・表など）に自動変換して投稿するため、エディタで「ブロックを解除」を押す必要はありません。HTML のまま投稿したい場合は `--no-gutenberg-blocks` を指定してください。

> **投稿するNoの指定**: `--nos "2-10,13-15"` のように範囲・カンマで投稿対象を絞れます（範囲指定時は `--limit 0` で全件処理を推奨）。
...
| `category` | 全記事のカテゴリを統一（既定 `ブログ`。空ならExcelの`WPカテゴリ`列を使用） |
| `nos` | 投稿するNoを範囲指定（例 `2-10,13-15`）。空なら全件。**範囲指定時は `limit=0` 推奨** |
| `input_file` | 制作管理表のパス |
...
- **Imunify360 に遮断される**: サーバー側のボット対策が断続的にリクエストを弾きます（HTTP 200 のまま拒否メッセージや HTML のチャレンジページを返す）。レンタルサーバーのため無効化できないので、遮断される前提で次の二段構えにしています。

  1. `wp_auto_post.py` が遮断を検知したら、間を空けて数回だけ試し直す（既定 10分 → 20分）。すぐ叩き直すとボット判定を強めるため、待ち時間は回を追うごとに伸ばします。回数を使い切ってもなお遮断されていれば、その実行を打ち切り以降のリクエストを一切送りません。
  2. 打ち切ったときだけリポジトリ直下に `.wp_blocked`（Git 管理外）を作り、`cybernote-security-news.yml` がそれを見て時間をおいて自分を実行し直します。遮断以外の失敗では作られないため、再実行の対象を遮断だけに絞れます。

  | 環境変数 / 変数 | 既定 | 内容 |
  |---|---|---|
  | `WP_BLOCK_RETRY_COUNT` | `2` | 遮断されたときに試し直す回数 |
  | `WP_BLOCK_RETRY_WAIT_SEC` | `600` | 1回目の待ち時間（秒）。2回目は2倍、3回目は3倍 |
  | `WP_USER_AGENT` | ブラウザ相当 | 遮断される場合にリポジトリ変数で上書き |
  | `vars.BLOCK_RERUN_WAIT_MIN` | `60` | ワークフロー再実行時に本処理の前に待つ分数 |
  | `vars.BLOCK_RERUN_MAX` | `3` | ワークフローの再実行回数の上限 |

  ワークフローの自動再実行にはシークレット `ACTIONS_PAT`（Fine-grained PAT / Actions: Read and write）が必要です（`GITHUB_TOKEN` では `workflow_dispatch` から新しい実行が起きない仕様のため）。未設定の場合は警告を出すだけで、手動再実行になります。
